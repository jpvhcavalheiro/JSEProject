...
public class TextInterface {
	...
	public static void createProduct() {
		...
		/*
		 * Iterator<Product> it = productRepository1.showAll(); while
	...
	public static void createShelf() {
		...
		do {
			...
			System.out.println(
					"Please insert the product id you want to insert in this shelf(if you dont want to insert anything please press none:");
			...
			if (!isAValidNumber(rentPrice1, "testDouble", 0.0, 32767.0)
				...
				|| !isAValidProductIdNumber(productIdInShelf1));
		if (!productIdInShelf1.equals("none")) {
			shelf1 = new Shelf(capacity1, Long.parseLong(productIdInShelf1), Double.parseDouble(rentPrice1));
		} else {
			shelf1 = new Shelf(capacity1, -1, Double.parseDouble(rentPrice1));
		}

		System.out.println(shelf1.getId());
		shelfRepository1.createEntity(shelf1);
		System.out.println(productRepository1.fetchEntityById(Long.parseLong(productIdInShelf1)).getShelvesIdList());
		productRepository1.fetchEntityById(Long.parseLong(productIdInShelf1)).getShelvesIdList().add(shelf1.getId());
		System.out.println(productRepository1.fetchEntityById(Long.parseLong(productIdInShelf1)).getShelvesIdList());
		/*
		 * Iterator<Shelf> it = shelfRepository1.showAll(); while (it.hasNext())
		 * { System.out.println(it.next().shelfToString()); } long shelf1Id =
		 * shelf1.getId();
		 * productRepository1.fetchEntityById(Long.parseLong(productIdInShelf1))
		 * .addNewShelfToShelvesList(shelf1Id);
		 */
	}
	...
	public static void consultProduct() {
		...
		do {
			...
			if (!isAValidProductIdNumber(productIdSearch) && !productIdSearch.equals("all")) {
				System.out.println("ERROR! Please insert a valid product id number!");
			}
		} while (!isAValidProductIdNumber(productIdSearch) && !productIdSearch.equals("all"));
	...
	public static void consultShelf() {
		...
		do {
			...
			shelfIdSearch = sc.nextLine();
			if (!isAValidShelfIdNumber(shelfIdSearch) && !shelfIdSearch.equals("all")) {
				System.out.println("Please insert a valid shelf id number!");
			}
		} while (!isAValidShelfIdNumber(shelfIdSearch) && !shelfIdSearch.equals("all"));
	...
	public static void removeProduct() {
		...
		if (confirmationOfProductIdToRemove.equals("yes")) {
			shelvesIdList1 = productRepository1.fetchEntityById(Long.parseLong(productIdToRemove)).getShelvesIdList();
			if (!shelvesIdList1.isEmpty()) {
				for (long shelfListItem : shelvesIdList1) {
					shelfRepository1.fetchEntityById(shelfListItem).setProductIdInShelf(-1);
				}
			}
	...
	public static void removeShelf() {
		...
		String confirmationOfShelfIdToRemove;
		do {
			System.out.println("Please insert the shelf id you want to remove:");
			shelfIdToRemove = sc.nextLine();
			if (!isAValidShelfIdNumber(shelfIdToRemove)) {
				System.out.println("There is no such shelf!");
			}
		} while (!isAValidShelfIdNumber(shelfIdToRemove));
		do {
			System.out.println("Are you sure you want to remove the product whose id is " + shelfIdToRemove
					+ "? Please insert yes or no.");
			confirmationOfShelfIdToRemove = sc.nextLine();
			if (!confirmationOfShelfIdToRemove.equals("yes") && !confirmationOfShelfIdToRemove.equals("no")) {
				System.out.println("Please insert yes or no!");
			}
		} while (!confirmationOfShelfIdToRemove.equals("yes") && !confirmationOfShelfIdToRemove.equals("no"));

		if (confirmationOfShelfIdToRemove.equals("yes")) {
			productRepository1
					.fetchEntityById(
							shelfRepository1.fetchEntityById(Long.parseLong(shelfIdToRemove)).getProductIdInShelf())
					.getShelvesIdList().remove(Long.parseLong(shelfIdToRemove));
	...
	/* se possa mudar a lista de prateleiras que contÃªm o produto */
	/* sÃ³ receba valores com sentido */
	public static void editProduct() {
		String productIdToEdit;
		String updatedProductDiscount;
		String updatedProductIva;
		String updatedProductPvp;
		do {
			System.out.println("Please insert the id of the product you want to edit:");
			productIdToEdit = sc.nextLine();
			if (!isAValidProductIdNumber(productIdToEdit)) {
				System.out.println("Error! There is no such product!");
			}
		} while (!isAValidProductIdNumber(productIdToEdit));
		...
		do {
			...
			updatedProductDiscount = sc.nextLine();
			System.out.println(
					"Initial IVA: " + productToEdit.getIva() + ". Please insert a new IVA or insert the same value:");
			updatedProductIva = sc.nextLine();
			System.out.println("Initial Price: " + productToEdit.getPvp()
					+ ". Please insert a new price or insert the same value:");
			updatedProductPvp = sc.nextLine();
			sc.nextLine();
			if (!isAValidNumber(updatedProductDiscount, "testInt", 0.0, 100.0)
					|| !isAValidNumber(updatedProductIva, "testInt", 0.0, 100.0)
					|| !isAValidNumber(updatedProductPvp, "testInt", 0.0, 32767.0)) {
				System.out.println("Please insert numbers within a reasonable range.");
			}
		} while (!isAValidNumber(updatedProductDiscount, "testInt", 0.0, 100.0)
				|| !isAValidNumber(updatedProductIva, "testInt", 0.0, 100.0)
				|| !isAValidNumber(updatedProductPvp, "testInt", 0.0, 32767.0));
		productToEdit.setDiscount(Integer.parseInt(updatedProductDiscount));
		productToEdit.setIva(Integer.parseInt(updatedProductIva));
		productToEdit.setPvp(Integer.parseInt(updatedProductPvp));
	...
	public static void editShelf() {
		String shelfIdToEdit;
		String updatedCapacity;
		String updatedProductIdInTheShelf;
		String updatedRentPrice;
		do {
			System.out.println("Please insert the id of the shelf you want to edit");
			shelfIdToEdit = sc.nextLine();
			if (!isAValidShelfIdNumber(shelfIdToEdit)) {
				System.out.println("Error! There is no such shelf!");
			}
		} while (!isAValidShelfIdNumber(shelfIdToEdit));
		...
		do {
			...
			updatedCapacity = sc.nextLine();
			System.out.println("Initially, the the id of product which is in this shelf is "
					+ shelfToEdit.getProductIdInShelf() + ". Please insert a new product id, the same id or none:");
			updatedProductIdInTheShelf = sc.nextLine();
			System.out.println("Initial rent price: " + shelfToEdit.getRentPrice()
					+ ". Please insert a new rent price or insert the same value:");
			updatedRentPrice = sc.nextLine();
			if(!isAValidNumber(updatedRentPrice, "testDouble", 0.0, 32767.0)
					|| (!isAValidProductIdNumber(updatedProductIdInTheShelf) && !updatedProductIdInTheShelf.equals("none"))){
				System.out.println("Error! Invalid input!");
			}
		} while (!isAValidNumber(updatedRentPrice, "testDouble", 0.0, 32767.0)
				|| (!isAValidProductIdNumber(updatedProductIdInTheShelf) && !updatedProductIdInTheShelf.equals("none")));
		shelfToEdit.setCapacity(updatedCapacity);
		shelfToEdit.setRentPrice(Integer.parseInt(updatedRentPrice));
		if(updatedProductIdInTheShelf.equals("none")){
			productRepository1.fetchEntityById(shelfToEdit.getProductIdInShelf()).getShelvesIdList().remove(Long.parseLong(shelfIdToEdit));
			shelfToEdit.setProductIdInShelf(-1);
		} else{
			productRepository1.fetchEntityById(shelfToEdit.getProductIdInShelf()).getShelvesIdList().remove(Long.parseLong(shelfIdToEdit));
			shelfToEdit.setProductIdInShelf(Long.parseLong(updatedProductIdInTheShelf));
			productRepository1.fetchEntityById(shelfToEdit.getProductIdInShelf()).getShelvesIdList().add(Long.parseLong(shelfIdToEdit));
		}
	...
	public static boolean isAValidShelfIdNumber(String StringToTest) {
		try {
			Long.parseLong(StringToTest);
			if (shelfRepository1.fetchEntityById(Long.parseLong(StringToTest)) == null) {
				return false;
			} else {
				return true;
			}
